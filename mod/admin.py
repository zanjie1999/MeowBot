...
async def adminTool(bot, context, data):
    if context:
        if context['message_type'] == 'private':
            # 私聊
            if context['user_id'] == myId:
                return await admin2at(bot, context, data['at'])
        elif context['message_type'] == 'group': 
            # 群聊
            return await at2admin(bot, context, data['at'])
            
# 我回复at  @xxx\r\n消息内容
async def admin2at(bot, context, at):
...
# 被at提醒我
async def at2admin(bot, context, at):
    if '[CQ:at,qq=' + str(context['self_id']) + ']' in context['message']:
        ...
        at[tip] = context
        # 提醒我
        tip = '在群 {} 被 @{}\n{}({}):\n{}'.format(context['group_id'], tip, context['sender']['nickname'], context['user_id'], context['message'])
        print(tip)
